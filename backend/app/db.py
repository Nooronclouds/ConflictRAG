...
def init_db():
    with _conn() as c:
        ...
        CREATE TABLE IF NOT EXISTS documents(
            ...
            date TEXT, type TEXT, size TEXT, status TEXT);
...
def list_documents(folder=None):
    with _conn() as c:
        base = "SELECT id,name,folder,date,type,size,status FROM documents"
        rows = c.execute(base + " WHERE folder=? ORDER BY name", (folder,)) if folder \
...
def add_document(name, folder=None, size="—"):
    with _conn() as c:
        c.execute("INSERT INTO documents(id,name,folder,date,type,size,status) VALUES(?,?,?,?,?,?,?)",
...
def get_conversation(cid):
    ...
    return {"id": conv["id"], "question": question, "response": response}
